	...
	public void run() {
		...
		while(isRun){
			...
			try {
				...
				ack_message_Header.setUsSsnId(FormatTransfer.lBytesToShort(Arrays.copyOfRange(message_Header, 0, 2)));
				ack_message_Header.setUsMsgId(FormatTransfer.lBytesToShort(Arrays.copyOfRange(message_Header, 2, 4)));
				ack_message_Header.setUiMsgSeq(FormatTransfer.lBytesToShort(Arrays.copyOfRange(message_Header, 4, 8)));
				ack_message_Header.setUcMsgType(message_Header[8]);
				ack_message_Header.setUcMsgOper(message_Header[9]);
				ack_message_Header.setUsBodySize(FormatTransfer.lBytesToShort(Arrays.copyOfRange(message_Header, 10, 12)));
				ack_body_b=new byte[ack_message_Header.getUsBodySize()];
				SocketService.is.read(ack_body_b);
				Message ack_message=new Message(ack_message_Header,ack_body_b);
