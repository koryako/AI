...
public class FansActivity extends Activity {
	...
	ImageView time_iv;
	ImageView wind_type_iv;
	ImageView shift_iv;
	ImageView shake_head_iv;
	String[] names = new String[] { "开/关", "定时", "风类", "换挡", "摇头" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.control_fans);
		...
		controlBroadcastReceiver = new ControlBroadcastReceiver(this,
				ApplType_E.APPL_TYPE_FAN.getVal());
		...
		// Toast.makeText(FansActivity.this, "请确认网络是否开启,连接失败",
		// Toast.LENGTH_LONG).show();
		// DisconnectionUtil.restart(FansActivity.this);
		// }
		...
		user_defined_Iv.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(FansActivity.this, UserDefinedActivity.class);
	...
	void init_Btn_Name() {
		...
		wind_type_iv = (ImageView) findViewById(R.id.wind_type_iv);
		shift_iv = (ImageView) findViewById(R.id.shift_iv);
		shake_head_iv = (ImageView) findViewById(R.id.shake_head_iv);

		switch_iv.setOnClickListener(new BtnListener(R.id.switch_iv));
		time_iv.setOnClickListener(new BtnListener(R.id.time_iv));
		wind_type_iv.setOnClickListener(new BtnListener(R.id.wind_type_iv));
		shift_iv.setOnClickListener(new BtnListener(R.id.shift_iv));
		shake_head_iv.setOnClickListener(new BtnListener(R.id.shake_head_iv));

		btn_Name.put(R.id.switch_iv, names[0]);
		btn_Name.put(R.id.time_iv, names[1]);
		btn_Name.put(R.id.wind_type_iv, names[2]);
		btn_Name.put(R.id.shift_iv, names[3]);
		btn_Name.put(R.id.shake_head_iv, names[4]);
	}
	...
	class BtnListener implements OnClickListener {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			if (!b) {
				Toast.makeText(FansActivity.this, "正在获取信息", Toast.LENGTH_LONG)
						.show();
				return;
			}
			int idx = checkIsExist(btn_Name.get(id));
			if (idx == -1) {
				Toast.makeText(FansActivity.this, "请添加此按钮的指令",
						Toast.LENGTH_LONG).show();
				Intent intent = new Intent();
				intent.putExtra("device", appl_S.getAppl_S());
				...
				intent.setClass(FansActivity.this, AddCmdActivity.class);
				...
				WriteUtil.write(MsgId_E.MSGID_CMD.getVal(), 1,
						...
						cmd_S.getCmd_S(), FansActivity.this);
				// } catch (IOException e) {
				// // TODO Auto-generated catch block
				// Toast.makeText(FansActivity.this, "请确认网络是否开启,连接失败",
				// Toast.LENGTH_LONG).show();
